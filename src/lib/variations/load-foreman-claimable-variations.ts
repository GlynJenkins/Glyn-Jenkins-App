...
export type ForemanClaimableVariationGroup = {
  groupKey:     string
  description:  string
  /** Management VO — single agreed foreman pay, not a daywork breakdown. */
  isFixedPay?:  boolean
  lines:        { id: string; workerName: string; amount: number }[]
  total:        number
}

/** Approved variations a foreman can include in a claim (worker lines + management fixed pay). */
export async function loadForemanClaimableVariations(
  ...
  for (const v of rawVariations ?? []) {
    ...
    const desc = (v.description ?? 'Variation').trim()
    const workerName = v.is_lump_sum
      ? desc
      : worker
